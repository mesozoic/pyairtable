...
def test_linked_record_can_be_saved(requests_mock, access_linked_records):
    ...
    address_url_re = re.escape(Address.get_table().urls.records + "?filterByFormula=")
    ...
    contact_url_re = re.escape(Contact.get_table().urls.records + "?filterByFormula=")
...
def test_undeclared_field(requests_mock, test_case):
    ...
    requests_mock.get(
        Address.get_table().urls.records,
        status_code=200,
        json={"records": [record]},
    )
    requests_mock.get(
